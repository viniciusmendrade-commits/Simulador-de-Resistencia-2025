...
import type { BuildingState, ComponentHealth } from '../types';
import { BuildingComponentId, MaterialTypeId, DisasterId } from '../types';
import { COMPONENT_CONFIG, NUMBER_OF_FLOORS, MIN_FLOOR_AREA, MAX_FLOOR_AREA } from '../constants';
...
const SeawallBarrier: React.FC<{ partProps: BuildingPartProps, isCollapsed: boolean }> = ({ partProps, isCollapsed }) => {
    const collapseDelay = (NUMBER_OF_FLOORS + 1) * 0.15;
    const collapseClass = isCollapsed ? 'animate-collapse' : '';
    const collapseStyle = { animationDelay: isCollapsed ? `${collapseDelay}s` : '0s' };
    return (
        <>
            {/* Left Barrier */}
            <div className={`absolute bottom-0 left-[12%] w-[10%] h-[6%] z-10 ${collapseClass}`} style={collapseStyle}>
                 <BuildingPart {...partProps} className="w-full h-full" style={{ clipPath: 'polygon(100% 0, 0 100%, 100% 100%)' }} />
            </div>
             {/* Right Barrier */}
            <div className={`absolute bottom-0 right-[12%] w-[10%] h-[6%] z-10 ${collapseClass}`} style={collapseStyle}>
...
const HydraulicSprings: React.FC<{ isCollapsed: boolean }> = ({ isCollapsed }) => {
    const Spring = () => (
    ...
    return (
        <div className={`absolute bottom-[0.5%] w-[90%] h-[4%] left-1/2 -translate-x-1/2 z-20 flex justify-around ${collapseClass}`} style={collapseStyle}>
...
const Building: React.FC<BuildingProps> = ({ buildingState, componentHealth, isSimulating, activeDisaster, floorArea, isCollapsed }) => {
  ...
  const getBuildingWidthPercent = () => {
    const minWidth = 45; // min width in %
    const maxWidth = 75; // max width in %
    const widthRange = maxWidth - minWidth;
    const areaRange = MAX_FLOOR_AREA - MIN_FLOOR_AREA;
    if (areaRange <= 0) return minWidth; // avoid division by zero
    const percentOfRange = (floorArea - MIN_FLOOR_AREA) / areaRange;
    return minWidth + (percentOfRange * widthRange);
  };
  const buildingWidthPercent = getBuildingWidthPercent();
  ...
  return (
    <div id="tutorial-building-area" className="w-full aspect-square max-w-[400px] md:max-w-[600px] relative flex items-end justify-center rounded-xl overflow-hidden">
        ...
        {tsunamiBarrierMaterial === MaterialTypeId.Medium && (
            <SeawallBarrier partProps={getPartProps(BuildingComponentId.TsunamiBarriers, 0)} isCollapsed={isCollapsed} />
        )}

        <div 
            className={`relative h-[90%] ${buildingShakeClass} ${buildingSwayClass} z-10 flex flex-col transition-[width] duration-500 ease-in-out`}
            style={{ width: `${buildingWidthPercent}%` }}
        >
        
        {/* SPDA System */}
        {spdaMaterial !== MaterialTypeId.Weak && (
          <div 
            className={`absolute top-[-8%] left-0 w-full h-[108%] z-20 pointer-events-none ${isCollapsed ? 'animate-collapse' : ''}`}
            style={{ animationDelay: isCollapsed ? '0s' : '0s' }}
          >
        ...
        {seismicDamperMaterial === MaterialTypeId.Strong && ( // Isoladores Sísmicos
          <div 
            className={`absolute bottom-[0%] w-full h-[2.5%] left-1/2 -translate-x-1/2 z-20 ${isCollapsed ? 'animate-collapse' : ''}`}
            style={{ animationDelay: isCollapsed ? `${(NUMBER_OF_FLOORS + 1) * 0.15}s` : '0s' }}
          >
              <BuildingPart 
        ...
        {seismicDamperMaterial === MaterialTypeId.Medium && ( // Amortecedores Hidráulicos (Molas)
             <HydraulicSprings isCollapsed={isCollapsed} />
        )}

        <div className="relative w-full flex-grow flex flex-col">
            <div className={`h-[8%] w-full flex flex-col relative ${isCollapsed ? 'animate-collapse' : ''}`}
                 style={{ animationDelay: isCollapsed ? `0s` : '0s' }}
            >
            ...
            {Array.from({ length: NUMBER_OF_FLOORS }).map((_, index) => {
                ...
                <div 
                    key={reversedIndex} 
                    className={`flex-grow flex flex-col relative ${isCollapsed ? 'animate-collapse' : ''}`}
                    style={{ animationDelay: isCollapsed ? `${(NUMBER_OF_FLOORS - reversedIndex) * 0.15}s` : '0s' }}
                >
                            ...
                            {Array.from({ length: 4 }).flatMap((_, colIndex) => {
                                ...
                                const windowElement = (
                                    <div key={`win-${reversedIndex}-${colIndex}`} className="flex-grow h-full flex flex-col bg-slate-800/10 w-full z-10">
                                        ...
                                        {reversedIndex === 0 && (colIndex === 1 || colIndex === 2) && buildingState[BuildingComponentId.Floor][0] !== MaterialTypeId.Empty ? (
            ...
            <div 
              className={`relative ${isCollapsed ? 'animate-collapse' : ''}`}
              style={{ animationDelay: isCollapsed ? `${(NUMBER_OF_FLOORS + 1) * 0.15}s` : '0s' }}
            >
              <BuildingPart {...getPartProps(BuildingComponentId.Floor, 0)} className="h-[5%] w-full" />
              <BuildingPart {...getPartProps(BuildingComponentId.Foundations, 0)} className="h-[4%] w-[98%] mx-auto" />
            </div>
        </div>
      </div>
       <style>{`
            ...
            @keyframes collapse {
                ...
                100% { transform: translateY(300px) rotate(10deg) scale(0.7); opacity: 0; }
            }
            .animate-collapse {
